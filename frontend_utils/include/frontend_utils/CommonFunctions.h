...
// template <typename T>
// Eigen::Matrix<T, 3, 3>
// PCLTwoPlaneVectorsFromNormal(const PointF& normal,
//                             Eigen::Matrix<T, 3, 1>& vec1,
...
void CalculateCovarianceFromNormals(const PointCloudF::ConstPtr& point_cloud,
  ...
  int enable_omp = (1 < k_num_threads);
#pragma omp parallel for schedule(dynamic, 1) if (enable_omp)
  for (int i = 0; i < point_cloud->points.size(); ++i) {
    cloud_covariances[i] = PCLTwoPlaneVectorsFromNormal<T>(
        point_cloud->points[i]); //, vec1, vec2);
  }
}
