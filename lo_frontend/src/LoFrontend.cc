...
std::vector<ros::AsyncSpinner> LoFrontend::setAsynchSpinners(ros::NodeHandle& _nh)
{
...
void LoFrontend::setImuSubscriber(ros::NodeHandle& _nh)
{
  ...
  ros::SubscribeOptions opts =
      ros::SubscribeOptions::create<sensor_msgs::Imu>("IMU_TOPIC",                                      // topic name
...
void LoFrontend::setLidarSubscriber(ros::NodeHandle& _nh)
{
  // Create options for subscriber and pass pointer to our custom queue
  ros::SubscribeOptions opts =
      ros::SubscribeOptions::create<PointCloudF>("LIDAR_TOPIC",                                      // topic name
...
bool LoFrontend::LoadParameters(const ros::NodeHandle& n)
{
  ...
  if (!pu::Get("b_debug_transforms", b_debug_transforms_))
    return false;
  if (!pu::Get("wait_for_odom_transform_timeout", wait_for_odom_transform_timeout_))
    return false;
...
bool LoFrontend::RegisterOnlineCallbacks(const ros::NodeHandle& n)
{
  ...
  if (b_sub_to_lsm_)
  {
    space_monitor_sub_ = nl_.subscribe("SPACE_MONITOR_TOPIC", 1, &LoFrontend::SpaceMonitorCallback, this);
  }
...
bool LoFrontend::CreatePublishers(const ros::NodeHandle& n)
{
  ...
  ros::NodeHandle nl(n);
  base_frame_pcld_pub_ = nl.advertise<PointCloudF>("base_frame_point_cloud", 10, false);
  lidar_callback_duration_pub_ = nl.advertise<std_msgs::Float64>("lidar_callback_duration", 10, false);
  dchange_voxel_pub_ = nl.advertise<std_msgs::Float64>("dchange_voxel", 10, false);

  change_leaf_size_pub_ =
      nl.advertise<std_msgs::Float64>(ros::this_node::getNamespace() + "/voxel_grid/change_leaf_size", 10, false);

  if (b_pub_odom_on_timer_)
  {
    ROS_INFO("Enabling odom_pub_timer_");
    odom_pub_timer_ = nl_.createTimer(odom_pub_rate_, &LoFrontend::PublishOdomOnTimer, this);
  }
  ...
  time_difference_pub_ = nl.advertise<std_msgs::Float64>("time_difference", 10, false);
  return true;
}
...
void LoFrontend::LidarCallback(const PointCloudF::ConstPtr& msg)
{
  ...
  {
    PublishOdometry(current_pose, localization_.GetLatestDeltaCovariance(), stamp);
  }
  ...
  Eigen::Matrix3d mat;
  mat << delta.rotation(0, 0), delta.rotation(0, 1), delta.rotation(0, 2), delta.rotation(1, 0), delta.rotation(1, 1),
...
void LoFrontend::PublishOdomOnTimer(const ros::TimerEvent& ev)
{
  ...
  Eigen::Matrix<double, 6, 6> covariance = localization_.GetLatestDeltaCovariance();
...

  {
    ...
    if (latest_odom_stamp > lidar_stamp && data_integration_mode_ >= 3 &&
        tf2_ros_odometry_buffer_.canTransform(base_frame_id_, latest_pose_stamp, base_frame_id_, latest_odom_stamp,
                                              bd_odom_frame_id_))
    {
      ...
      t = tf2_ros_odometry_buffer_.lookupTransform(base_frame_id_, latest_pose_stamp, base_frame_id_, latest_odom_stamp,
                                                   bd_odom_frame_id_);
    }
    else
    {
...
void LoFrontend::PublishOdometry(const geometry_utils::Transform3& odometry,
                                 const Eigen::Matrix<double, 6, 6>& covariance, const ros::Time stamp)
{
...
void LoFrontend::CheckImuFrame(const ImuConstPtr& imu_msg)
{
  if (b_convert_imu_to_base_link_frame_)
  {
    ...
    {
      ROS_ERROR(
          "Received imu_msg is not expressed in imu frame, but an imu to "
  ...
  else
  {
    ...
    {
      ROS_ERROR(
          "Received imu_msg is not expressed in base_link frame, but an "
...
bool LoFrontend::LoadCalibrationFromTfTree()
{
  ...
  ROS_WARN_DELAYED_THROTTLE(2.0, "Waiting for \'%s\' and \'%s\' to appear in tf_tree...", imu_frame_id_.c_str(),
                            base_frame_id_.c_str());
  tf::StampedTransform imu_T_base_transform;
  try
  {
    imu_T_base_listener_.waitForTransform(imu_frame_id_, base_frame_id_, ros::Time(0), ros::Duration(2.0));

    imu_T_base_listener_.lookupTransform(imu_frame_id_, base_frame_id_, ros::Time(0), imu_T_base_transform);
...
//  gtsam::Rot3 r(pose.rotation(0, 0), pose.rotation(0, 1), pose.rotation(0, 2), pose.rotation(1, 0), pose.rotation(1,
//  1),
//                pose.rotation(1, 2), pose.rotation(2, 0), pose.rotation(2, 1), pose.rotation(2, 2));
...
{
  Eigen::Quaterniond imu_quaternion = Eigen::Quaterniond(double(imu_msg.orientation.w), double(imu_msg.orientation.x),
                                                         double(imu_msg.orientation.y), double(imu_msg.orientation.z));
  if (b_convert_imu_to_base_link_frame_)
  {
...
{
  bool change = false;
  double dchange_voxel = double_param.value * (static_cast<double>(msg->points.size()) /
  ...
  if (std::abs(double_param.value - dchange_voxel) > 0.01 or counter_voxel_ % 20 == 0)
  {
...
Eigen::Matrix3d LoFrontend::GetImuYawDelta()
{
  ...
  tf::Quaternion q(imu_quaternion_change_.x(), imu_quaternion_change_.y(), imu_quaternion_change_.z(),
                   imu_quaternion_change_.w());
  tf::Matrix3x3 m(q);
  m.getRPY(roll, pitch, yaw);
  ROS_INFO_STREAM("LoFrontend::GetImuYawDelta - Yaw delta from IMU is " << yaw * 180.0 / M_PI << " deg");
...
template <typename T1, typename T2>
bool LoFrontend::GetMsgAtTime(const ros::Time& stamp, T1& msg, const T2& buffer) const
{
  if (buffer.size() == 0)
  {
    return false;
...
bool LoFrontend::IntegrateOdom(const ros::Time& stamp)
{
  Odometry odometry_msg;
  {
    ...
    {
      ROS_WARN(
          "Unable to retrieve odometry_msg from odometry_buffer_ "
...
bool LoFrontend::IntegrateInterpolatedOdom(const ros::Time& stamp)
{
...

  {
    ...
    if (tf2_ros_odometry_buffer_.canTransform(base_frame_id_, previous_stamp_, base_frame_id_, stamp_transform_to_,
                                              bd_odom_frame_id_))
    {
      have_odom_transform = true;
      t = tf2_ros_odometry_buffer_.lookupTransform(base_frame_id_, previous_stamp_, base_frame_id_, stamp_transform_to_,
                                                   bd_odom_frame_id_);
    }
  }
...
bool LoFrontend::IntegrateImu(const ros::Time& stamp)
{
  Imu imu_msg;
  {
    ...
    {
      ROS_WARN(
          "Unable to retrieve imu_msg from imu_buffer_ "
          "given lidar timestamp");
      return false;
    }
  }
