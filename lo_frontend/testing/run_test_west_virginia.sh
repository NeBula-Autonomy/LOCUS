...
#### DOCUMENTATION: Here we compare blam_slam/odometry_integrated_estimate against lo_frontend/odometry_integrated_estimate when using external IMU Attitude ####
...
#### Play BAGFILE 
tmux send-keys -t 1 "sleep 2; rosparam set /use_sim_time true && rosbag play -r 1.0 ~/bags/west_virginia/husky1_* --clock --pause /velodyne_points:=/$ROBOT_NAME/velodyne_points" C-m

#### Launch LO_FRONTEND 
tmux send-keys -t 2 "sleep 2; roslaunch lo_frontend lo_frontend_imu.launch robot_namespace:=$ROBOT_NAME" C-m
...
tmux send-keys -t 5 "sleep 2; rosrun rqt_multiplot rqt_multiplot --multiplot-run-all --clear-config --multiplot-config $(rospack find lo_frontend)/testing/rqt_multiplot_imu_west_virginia.xml" C-m
tmux send-keys -t 6 "sleep 2; rosrun tf2_ros static_transform_publisher 0 0 -0.08 0 0 0 1 /velodyne $ROBOT_NAME/base_link" C-m
tmux send-keys -t 7 "sleep 5; rosrun topic_tools transform /vn100/imu /vn100/imu/rpy  geometry_msgs/Vector3 'tf.transformations.euler_from_quaternion([m.orientation.x, m.orientation.y, m.orientation.z, m.orientation.w])' --import tf" C-m
