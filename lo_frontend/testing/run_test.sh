...
##### Launch LION 
tmux send-keys -t 2 "sleep 2; roslaunch localizer_lion localizer_lion.launch imu_topic:=$IMU_TOPIC pose_topic:=$POSE_TOPIC" C-m 
...
tmux send-keys -t 4 "sleep 2; roslaunch quat2eul quat2eul.launch" C-m

#### Execute .py script to remove offset from visualization of groundtruth 
tmux send-keys -t 5 "sleep 5; python $(rospack find lo_frontend)/testing/subtract_offset_from_groundtruth.py" C-m 

#### Run rqt for visualization 
tmux send-keys -t 6 "sleep 2; rosrun rqt_multiplot rqt_multiplot --multiplot-run-all --clear-config --multiplot-config $(rospack find lo_frontend)/testing/rqt_multiplot.xml" C-m
