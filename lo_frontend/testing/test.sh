...
#### Panel structure ####
## Create three columns in the whole panel
...
tmux send-keys -t 2 "sleep 2; python $(rospack find lo_frontend)/testing/subtract_offset_from_groundtruth.py" C-m # LIDAR RATE
tmux send-keys -t 3 "source $WORKSPACE/devel/setup.bash; sleep 2; roslaunch imu_complementary_filter complementary_filter.launch" C-m 
...
tmux send-keys -t 7 "sleep 2; source $WORKSPACE/devel/setup.bash;  rosrun rqt_multiplot rqt_multiplot --multiplot-run-all --clear-config --multiplot-config /home/matteo/andrea_ws/src/localizer_lo_frontend/lo_frontend/testing/rqt_multiplot.xml" C-m
tmux send-keys -t 8 "sleep 2; rosrun tf2_ros static_transform_publisher 0 0 -0.08 0 0 0 1 /velodyne $ROBOT_NAME/base_link" C-m
tmux send-keys -t 9 "sleep 2; rosrun topic_tools transform /Robot_7/pose /Robot_7/pose/rpy  geometry_msgs/Vector3 'tf.transformations.euler_from_quaternion([m.pose.orientation.x, m.pose.orientation.y, m.pose.orientation.z, m.pose.orientation.w])' --import tf" C-m
tmux send-keys -t 6 "sleep 10; rosrun topic_tools transform /$ROBOT_NAME/lo_frontend/odometry_integrated_estimate /$ROBOT_NAME/lo_frontend/odometry_integrated_estimate/rpy geometry_msgs/Vector3 'tf.transformations.euler_from_quaternion([m.pose.orientation.x, m.pose.orientation.y, m.pose.orientation.z, m.pose.orientation.w])' --import tf" C-m
# Position 
# Velocity 
# Imu bias
# Imu gyro 
