...
tmux send-keys -t 6 "sleep 2; rosparam set /use_sim_time true && rosbag play -r 1.0 ~/bags/lo_frontend/slow1.bag --clock --pause /velodyne_points:=/$ROBOT_NAME/velodyne_points" C-m

##### Play rosbag
tmux send-keys -t 1 "sleep 3; rosbag play -r 1.0 ~/bags/lo_frontend/slow1.bag --clock --pause" C-m 

tmux send-keys -t 2 "sleep 5; python /home/matteo/andrea_ws/src/localizer_lo_frontend/lo_frontend/testing/subtract_offset_from_groundtruth.py" C-m # LION pose input rate
