...
bool Get(const std::string& s, M& p)
{
  ...
  {
    ROS_WARN("%s: Failed to search for parameter '%s'.", name.c_str(), s.c_str());
    return false;
  }
...
bool Get(const std::string& s, M& p, M def)
{
  ...
  if (!ros::param::search(s, r))
  {
    ROS_DEBUG("%s: Failed to search for parameter '%s', using default.", name.c_str(), s.c_str());
    p = def;
    ret = false;
  }
  ...
  {
    ROS_DEBUG("%s: Missing required parameter '%s', using default.", name.c_str(), s.c_str());
    p = def;
    ret = false;
  }
  ...
  {
    ROS_DEBUG("%s: Failed to get parameter '%s', using default.", name.c_str(), s.c_str());
    p = def;
    ret = false;
  }
