...
bool PointCloudIkdTreeMapper::RegisterCallbacks(const ros::NodeHandle& n)
{
  ...
  incremental_map_pub_ = nl.advertise<PointCloudF>("octree_map_updates", 10, true);
  ...
  boxes_vis_pub_ = nl.advertise<visualization_msgs::MarkerArray>("points_inside_boxes_to_remove", 10, true);

  search_ikdtree_pub_ = nl.advertise<std_msgs::Float64>("search_in_map", 10, false);
  delete_ikdtree_pub_ = nl.advertise<std_msgs::Float64>("delete_from_map", 10, false);
  adding_ikdtree_pub_ = nl.advertise<std_msgs::Float64>("adding_to_map", 10, false);

  insert_points_init_pub_ = nl.advertise<std_msgs::UInt64>("no_insert_points_pub", 10, false);

  nearest_points_init_pub_ = nl.advertise<std_msgs::UInt64>("no_nearest_points", 10, false);
...
visualization_msgs::Marker PointCloudIkdTreeMapper::CreateBoxToVisualize(const Eigen::Vector3f& current_position,
                                                                         int id) const
{
...
void PointCloudIkdTreeMapper::UpdateBoxesLocationWithRespectToTheRobot(const geometry_utils::Transform3& current_pose)
{
  for (size_t i = 0; i < box_filter_centers_.size(); ++i)
  {
    box_filters_[i].vertex_min[0] =
        static_cast<float>(current_pose.translation.data[0]) + box_filter_centers_[i][0] - box_filter_size_;
    box_filters_[i].vertex_min[1] =
        static_cast<float>(current_pose.translation.data[1]) + box_filter_centers_[i][1] - box_filter_size_;
    box_filters_[i].vertex_min[2] =
        static_cast<float>(current_pose.translation.data[2]) + box_filter_centers_[i][2] - box_filter_size_;
  ...
  if (boxes_vis_pub_.getNumSubscribers() != 0)
  {
    ...
    {
      auto box_vis = CreateBoxToVisualize(current_pose.translation.Eigen().cast<float>() + box_filter_centers_[i], i);
...
bool PointCloudIkdTreeMapper::InsertPoints(const PointCloudF::ConstPtr& points, PointCloudF* incremental_points)
{
  if (!initialized_)
  {
...
bool PointCloudIkdTreeMapper::ApproxNearestNeighbors(const PointCloudF& points, PointCloudF* neighbors)
{
  if (!initialized_)
  {
    ROS_ERROR("%s: Not initialized.", name_.c_str());
...
void PointCloudIkdTreeMapper::PublishMap()
{
  if (map_pub_.getNumSubscribers() > 0 || !b_publish_only_with_subscribers_)
  {
    if (initialized_ && map_updated_)
    {
      ...
      publish_thread_ = std::thread(&PointCloudIkdTreeMapper::PublishMapThread, this);
    }
  }
}
...
void PointCloudIkdTreeMapper::PublishMapFrozen()
{
  if (initialized_ && map_frozen_pub_.getNumSubscribers() > 0)
  {
    ...
    publish_frozen_thread_ = std::thread(&PointCloudIkdTreeMapper::PublishMapFrozenThread, this);
  }
}
...
void PointCloudIkdTreeMapper::PublishMapUpdate(const PointCloudF& incremental_points)
{
...
void PointCloudIkdTreeMapper::SetBoxFilterSize(const int box_filter_size)
{
  ...
  ROS_INFO_STREAM("Setting up box filter with half length: " << box_filter_size);

  box_filters_.push_back(BoxPointType({ Eigen::Vector3f{ -box_filter_size_, -box_filter_size_, -box_filter_size_ },
  ...
  box_filter_centers_.push_back(Eigen::Vector3f(0, 2.0f * box_filter_size_, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(0, -2.0f * box_filter_size_, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(-2.0f * box_filter_size_, 0.0f, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(2.0f * box_filter_size_, 0.0f, 0.0f));
  ...
  box_filter_centers_.push_back(Eigen::Vector3f(2.0f * box_filter_size_, 2.0f * box_filter_size_, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(-2.0f * box_filter_size_, -2.0f * box_filter_size_, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(-2.0f * box_filter_size_, 2.0f * box_filter_size_, 0.0f));
  box_filter_centers_.push_back(Eigen::Vector3f(2.0f * box_filter_size_, -2.0f * box_filter_size_, 0.0f));
...
void PointCloudIkdTreeMapper::Refresh(const geometry_utils::Transform3& current_pose)
{
  // ROS_WARN_STREAM(
  //     "##################################################################");
