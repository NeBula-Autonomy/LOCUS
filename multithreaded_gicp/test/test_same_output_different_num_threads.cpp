...
int main(int argc, char **argv) {
  ...
  for (int i = 1; i < 9; i++) {
    ...
    if (T == original_gicp_transform) {
      std::cout << "SUCCESS: Transform matches original GICP transform" << std::endl;
    } else {
      std::cerr << "FAILURE: Transform does not match original GICP transform" << std::endl;
      success = false;
    }

    if (fitness_score == original_gicp_fitness_score) {
      std::cout << "SUCCESS: Fitness score matches original GICP fitness score" << std::endl;
    } else {
      std::cerr << "FAILURE: Fitness score does not match original GICP fitness score" << std::endl;
      success = false;
    }
    std::cout << std::endl;
  }
